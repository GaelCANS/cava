@extends('frontoff.app')

@section('content')
    <div class="row">
        <div class="col-md-8 text-center mx-auto px-4 mb-5">
            <h3>VOTRE NOUVEL ESPACE COLLABORATIF AGENCE NEUFCHÂTEL-EN-BRAY</h3>
            Note moyenne

        </div>
    </div>
    <div class="row">
        <div id="question-box" class="col-md-6 text-center mx-auto">
            <div class="head-result-box">
                <span class="number float-left">1</span>
                <span class="emote float-left">
                    <img src="data:image/svg+xml;utf8;base64," />
                </span>
                <span class="graph float-right"><i class="fa phpdebugbar-fa-area-chart"></i></span>
            </div>
            <div style="clear:both;"></div>
            <div class="col-md-12">
                <h4 class="mt-3 mb-4">Êtes-vous satisfait par votre nouvel espace collaboratif ?</h4>
                <!--<input type="range" min="0" max="5" step="1" name="result" value="{{ rand(0,5) }}"  >-->
                <div class="barGraph">
                    <ul class="graph">
                      <span class="graph-barBack">
                        <li class="graph-bar" data-value="4"></li>
                      </span>

                      <span class="graph-barBack">
                        <li class="graph-bar total" data-value="1"></li>
                      </span>


                    </ul>
                    <span class="barGraph-check"></span>
                    <span class="barGraph-check ml40"></span>
                    <span class="barGraph-check ml60"></span>
                    <span class="barGraph-check ml80"></span>
                </div>
                <span class="text-muted">Voir les commentaires du gestionnaire</span>
            </div>
        </div>
    </div>
    <div class="row">
        <div id="question-box" class="col-md-6 text-center mx-auto">
            <div class="head-result-box">
                <span class="number float-left">1</span>
                <span class="emote float-left">
                    <img src="data:image/svg+xml;utf8;base64," />
                </span>
                <span class="graph float-right"><i class="fa phpdebugbar-fa-area-chart"></i></span>
            </div>
            <div style="clear:both;"></div>
            <div class="col-md-12">
                <h4 class="mt-3 mb-4">Êtes-vous satisfait par votre nouvel espace collaboratif ?</h4>
                <!--<input type="range" min="0" max="5" step="1" name="result" value="{{ rand(0,5) }}"  >-->
                <div class="barGraph">
                    <ul class="graph">
                      <span class="graph-barBack">
                        <li class="graph-bar" data-value="4"></li>
                      </span>

                      <span class="graph-barBack">
                        <li class="graph-bar total" data-value="1"></li>
                      </span>


                    </ul>
                    <span class="barGraph-check"></span>
                    <span class="barGraph-check ml40"></span>
                    <span class="barGraph-check ml60"></span>
                    <span class="barGraph-check ml80"></span>
                </div>
                <span class="text-muted">Voir les commentaires du gestionnaire</span>
            </div>
        </div>
    </div>




@endsection