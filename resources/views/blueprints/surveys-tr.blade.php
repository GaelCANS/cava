<tr data-id="{{$survey->id}}" data-link="{{route('participants-survey' , $survey->id)}}" class="@if ($survey->current) active @endif">
    ...
    <td>
        <a href="{{route('comments-survey' , array($survey->id))}}" class="open-comments" data-from="0" data-range="3" title="Voir les commentaires de l'itération"><button type="button" class="btn btn-outline-secondary icon-btn"><i class="mdi mdi-comment"></i></button></a>
